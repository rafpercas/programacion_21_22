...
public class Consultas {
    ...
    try {
      ...
      ResultSet rs =
          statement.executeQuery(
              "select productName,buyPrice from products where buyPrice > " + precioMin);
  ...
  public void InfoPagos() {
    try {
      ...
      ResultSet rs =
          statement.executeQuery(
              "select customerName,checkNumber, paymentDate,amount from customers c "
                  + "join payments p on p.customerNumber = c.customerNumber where c.customerNumber = "
                  + numCliente);
  ...
  public void informeCategoria() {
    try {
      ...
      ResultSet rs =
          statement.executeQuery(
              "select p.productName,od.quantityOrdered, od.priceEach from products p join orderdetails od on p.productCode = od.productCode "
                  + "where p.productLine = '"
                  + nombreCategoria
                  + "'");
